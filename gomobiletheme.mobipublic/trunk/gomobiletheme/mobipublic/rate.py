...
def getId(context, create):
    sdm = context.session_data_manager
    session_id = sdm.getBrowserIdManager().getBrowserId(create=create)
    return session_id
...
def loveIt(context, userid=None):
    ...
    if not userid:
        ...
        userid = getId(context, create=True)
...
def getMyVote(context, userid=None):
    ...
    if not userid:
        userid = getId(context, create=False)        
