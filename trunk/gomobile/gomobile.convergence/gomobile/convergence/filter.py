...
class ConvergedMediaFilter:
    ...
    def isConvergedContent(self, content):
        ...
        if not hasattr(content, "Schema"):
            # Portal, criteria,other "special objects"
            return False
    
        schema = content.Schema()
        
        # At 
        return "contentMedias" in schema
    ...
    def getContentMedia(self, content):
        ...
        if not hasattr(content, "Schema"):
            raise RuntimeError("Not AT object")
        ...
        return schema["contentMedias"].get(content)
    ...
    def setContentMedia(self, content, strategy):
        ...
        if not "contentMedias" in schema:
            raise RuntimeError("extender has not patched the object properly")
        
        field = schema["contentMedias"]
        
            # Update mobile schema in the content object
        if not field.checkPermission("w", content):
            raise Unauthorized("Setting content media not allowed")
        
        # TODO: Got traversing errors when using set() directly, ugly workaround, remove!
        content.update(contentMedias=strategy)
