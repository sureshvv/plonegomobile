...
from gomobile.convergence.mobileoverrides.base import IMobileOverrideStorage
...
class TestMobileOverrides(BaseTestCase):
    ...
    def test_has_overrides(self):
        ...
        self.create_doc()
        doc = self.portal.doc
        overrider = IMobileOverrider(doc)
        assert overrider != None

        assert overrider.schema != None
        assert list(overrider._getOverrideFieldNames()) == ["Title", "Description", "getText"]

        # Set one override enabled and check it is read properly
        storage = IMobileOverrideStorage(doc)
        storage.enabled_overrides = ["Title"]
        assert overrider._isOverrideEnabled("Title") == True

        # Set overriden title
        storage.Title = u"Foobar"
        assert overrider._getOverrideOrOrignal("Title") == u"Foobar"
    ...
    def test_has_no_overrides(self):
        ...
        try:
            overrider = IMobileOverrider(event)
